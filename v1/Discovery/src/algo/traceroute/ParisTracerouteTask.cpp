...
ParisTracerouteTask::ParisTracerouteTask(Environment *e, 
                                         SubnetSite *ss, 
                                         unsigned short lbii, 
                                         unsigned short ubii, 
                                         unsigned short lbis, 
...
void ParisTracerouteTask::run()
{
    ...
    while(probeTTL > 0)
    {
        ...
        if(rplyAddress == InetAddress(0))
        {
            ...
            /*
             * N.B.: there is no third attempt after a second timeout. Next steps, however, might 
             * reprobe the target with the appropriate TTL after a delay to see if there is still 
             * a timeout (cf. AnonymousChecker class). This is motivated by the fact that some IPs 
             * seem to be anonymized, either by a firewall, either by themselves (e.g. through 
             * rate-limiting mechanisms), when they receive many probes.
             */
