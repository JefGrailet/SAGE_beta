 ...
 * through offline analysis. To do so, it carefully analyzes the current routes to isolate those 
 * which require additionnal probing (if several incomplete routes are similar and could be fixed 
 * offline, then only one probe is needed) then spread the work among several probing threads (see 
 * the class AnonymousCheckUnit).
 */
...
public:
    ...
    AnonymousChecker(Environment *env);
    ~AnonymousChecker();
...
private:
    ...
    // Pointer to the environment singleton (provides access to subnets and probing parameters)
    Environment *env;
