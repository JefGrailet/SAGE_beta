...
SubnetInferrer::SubnetInferrer(Environment *e, 
                               unsigned short li, 
                               unsigned short ui, 
                               unsigned short ls, 
                               unsigned short us):
env(e)
{
    ...
    debug = env->debugMode();
    verbose = debug || env->getDisplayMode() == Environment::DISPLAY_MODE_VERBOSE;
                                              ...
                                              bool useLowerBorderAsWell)
{
    ...
    try
    {
         ...
         * "IPBlocksToAvoid" set found in the Environment class. However, the TTL must be the same 
         * as the Pivot TTL for this range to exclude the destination IP for sure, hence the need 
         * to go through populateRecords(). To signal this to the calling environment, this method 
         * returns the subnet from IPBlocksToAvoid which contains the destination IP. There is no 
         * ambiguity since it has a refinement status other than NOT_PREPARED_YET.
         */
        
        if(siteRecord != NULL)
        {
            SubnetSiteSet *toAvoid = env->getIPBlocksToAvoid();
            SubnetSite *ss = toAvoid->getEncompassingSubnet(destinationAddress, 
                                     ...
                                     bool useLowerBorderAsWell)
{
    ...
    /*
     * Registers the TTL for this IP in the IP dictionary. As of April 2018, the code also checks 
     * whether there is an expected TTL value as a consequence of the previous inference of a 
     * subnet encompassing this target IP. If this expected TTL exists, this will stop the subnet 
     * inference for this specific target (via an exception) and set the TTL value for the target 
     * IP in the dictionary. It will later be added to the encompassing subnet thanks to the 
     * refinement by filling (modifying the encompassing subnet now can be dangerous due to 
     * multithreading).
     *
     * Also note that there is no lock to access the IP dictionary. Indeed, during scanning, no 
     * new IP is added, and none is deleted; it is only accessed for reading. It is also highly 
     * unlikely multiple threads call setTTL() on a same IP entry object, because this would 
     * mean there is a duplicate target somewhere.
     */
    ...
    if(IPEntry != NULL)
    {
        ...
        unsigned char eTTL = IPEntry->getExpectedTTL();
        if(eTTL != IPTableEntry::NO_KNOWN_TTL)
        {
            std::string msg = "";
            ...
            throw ExpectedTTLException(dst, msg);
        }
    }
                ...
                if(!probedIPset.contains(currentAddress))
                {
                    ...
                    if(!newRecorddTTLPlus1->isAnonymousRecord())
                    {
                        ...
                        if(growSubnet_Rule_1_B(newRecorddTTLPlus1, subnetPrefix))
                        {
                             ...
                             * different device. A subsequent step aims at providing a much more 
                             * elaborated, complete alias resolution method. It also provides a 
                             * subnet refinement phase to ensure the presence of one or several 
                             * contra-pivot candidates in each subnet and evaluates the soundness 
                             * of an inferred subnet on that basis.
                             *
