...
SubnetRefiner::SubnetRefiner(Environment *env)
{
    this->env = env;
    ...
    {
        case Environment::DISPLAY_MODE_SLIGHTLY_VERBOSE:
        case Environment::DISPLAY_MODE_VERBOSE:
            this->printSteps = true;
            break;
        case Environment::DISPLAY_MODE_DEBUG:
            this->printSteps = true;
            this->debug = true;
            break;
        default:
...
void SubnetRefiner::expand(SubnetSite *ss)
{
    ...
    // Computes the initial borders as well as the mask of known IPs
    InetAddress initLowerBorder, initUpperBorder;
    unsigned short initSize = (unsigned int) pow(2.0, (double) (32 - (unsigned int) initialPrefix));
    if(initialPrefix == 32)
    {
        initLowerBorder = ss->getPivotAddress();
        initUpperBorder = initLowerBorder;
    }
    else
    {
    ...
    bool knownIPs[initSize];
    for(unsigned short i = 0; i < initSize; i++)
        knownIPs[i] = false;
    
    list<SubnetSiteNode*> *IPsList = ss->getSubnetIPList();
    ...
    for(list<SubnetSiteNode*>::iterator it = IPsList->begin(); it != IPsList->end(); ++it)
    {
        ...
        {
            knownIPs[(*it)->ip.getULongAddress() - initLowerBorder.getULongAddress()] = true;
            nbValidIPs++;
        }
    }
    ...
    while(prefix >= SubnetRefiner::LOWEST_PREFIX_ALLOWED)
    {
        ...
        for(InetAddress i = lowerBorder; i <= upperBorder; i++)
        {
            ...
            {
                if(knownIPs[i.getULongAddress() - initLowerBorder.getULongAddress()])
                {
                    continue;
                }
            }
        ...
        {
            list<InetAddress> *candidates = dispatcher->getResponsiveIPs();
            ...
            else
            {
                ...
                list<InetAddress>::iterator listBegin = candidates->begin();
                list<InetAddress>::iterator listEnd = candidates->end();
                for(list<InetAddress>::iterator it = listBegin; it != listEnd; ++it)
                {
                    if(printSteps)
                        (*out) << (*it) << "\n";
    ...
    // If we got out of the loop, this means we could not find a contra-pivot even with /20 prefix.
    if(printSteps)
    {
    ...
    // Creates equivalent /20 subnet in "toAvoid" subnet set
...
void SubnetRefiner::finalize(SubnetSite *ss)
{
    ostream *out = env->getOutputStream();

    // Like before, computes the initial borders and a mask of known IPs
    unsigned char ssPrefix = ss->getInferredSubnetPrefixLength();
    InetAddress lowerBorder, upperBorder;
    unsigned short sizeArray = (unsigned int) pow(2.0, (double) (32 - (unsigned int) ssPrefix));
    
    // By design, the subnet will be at the very least a /31. We don't have to check for /32's.
    NetworkAddress na = ss->getInferredNetworkAddress();
    lowerBorder = na.getLowerBorderAddress();
    upperBorder = na.getUpperBorderAddress();
    
    bool knownIPs[sizeArray];
    ...
    for(list<SubnetSiteNode*>::iterator it = IPsList->begin(); it != IPsList->end(); ++it)
    {
        unsigned long offset = (*it)->ip.getULongAddress() - lowerBorder.getULongAddress();
        if(offset >= 0 && offset < sizeArray) // Just in case
            knownIPs[(*it)->ip.getULongAddress() - lowerBorder.getULongAddress()] = true;
    }
    
    /*
     * Lists IPs encompassed by the subnet present in the dictionnary (therefore at pre-scanning), 
     * but not in the node list.
     */
    
    IPLookUpTable *dict = env->getIPTable();
    list<InetAddress> possiblePivots;
    for(InetAddress i = lowerBorder; i <= upperBorder; i++)
    {
        IPTableEntry *IP = dict->lookUp(i);
        if(IP != NULL && !knownIPs[i.getULongAddress() - lowerBorder.getULongAddress()])
            possiblePivots.push_back(i);
    }
    
    // Does nothing if no new IPs to probe.
    if(possiblePivots.size() == 0)
        return;
    
    if(printSteps)
    {
        (*out) << "Finalizing subnet by probing previous responsive IPs encompassed by the ";
        (*out) << "subnet but not listed (TTL = pivot TTL)." << endl;
    }
    
    // Sends the probes
    unsigned char pivotTTL = ss->getShortestTTL() + 1;
    ProbesDispatcher *dispatcher = new ProbesDispatcher(env, possiblePivots, pivotTTL);
    unsigned short result = 0;
    try
    {
        result = dispatcher->dispatch();
    ...
    catch(StopException &se)
    {
        ...
        if(printSteps)
        {
            (*out) << "Critical failure occurred while probing (finalizing expanded subnet).\n";
            (*out) << "The subnet will not be finalized.\n" << endl;
        }
        else
        {
            (*out) << "Critical failure occurred while probing (subnet finalization)." << endl;
        }
    ...
    if(result == ProbesDispatcher::FOUND_RESPONSIVE_IPS)
    {
        ...
        if(printSteps)
        {
            size_t nbNewNodes = candidates->size();
            if(nbNewNodes > 1)
                (*out) << "Discovered " << nbNewNodes << " additionnal pivot interfaces.\n" << endl;
            else
                (*out) << "Discovered one additionnal pivot interface.\n" << endl;
        }
    }
    else if(printSteps)
    {
        (*out) << "No additionnal subnet interface was discovered at pivot TTL.\n" << endl;
    }
...
void SubnetRefiner::fill(SubnetSite *ss)
{
    ...
    // Tests some exceptional cases before getting to work
    InetAddress lowerBorder, upperBorder;
    list<SubnetSiteNode*> *IPsList = ss->getSubnetIPList();
    ...
    else if(prefixLength == 0) // For some reason, this occurred once on PlanetLab...
    {
        if(printSteps)
    ...
    for(InetAddress i = lowerBorder; i <= upperBorder; i++)
    {
        ...
        {
            IPTableEntry *iEntry = dict->lookUp(i);
             ...
             * this IP). Having no TTL means the IP was most probably unresponsive at scanning; 
             * in that case, the pivot TTL is associated to it by default.
             */
