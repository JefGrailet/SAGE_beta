...
AliasHintCollector::AliasHintCollector(Environment *env)
{
    this->env = env;
    tokenCounter = 1;
...
void AliasHintCollector::collect()
{
    ...
    for(list<InetAddress>::iterator it = backUp1.begin(); it != backUp1.end(); ++it)
    {
        ...
        if(res != IPIDTuples.end())
        {
            ...
            // Resets AR hints if some already exist
            if(targetEntry->hasIPIDData())
                targetEntry->resetARHints();
...
void AliasHintCollector::collect(Neighborhood *n)
{
