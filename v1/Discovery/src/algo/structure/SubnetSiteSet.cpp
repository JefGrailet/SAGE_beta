...
list<SubnetSite*> SubnetSiteSet::postProcessNewSubnets(list<SubnetSite*> *discovered)
{
    list<SubnetSite*> incomplete;
    for(list<SubnetSite*>::iterator it = siteList.begin(); it != siteList.end(); ++it)
    {
        SubnetSite *ss = (*it);
        if(ss->getStatus() != SubnetSite::NOT_PREPARED_YET)
            continue;
        
        ss->markAsNewSubnet();
        ss->prepareForRefinement();
        switch(ss->getStatus())
        {
            case SubnetSite::INCOMPLETE_SUBNET:
                siteList.erase(it--);
                incomplete.push_back(ss);
                discovered->push_back(ss);
                break;
...
list<SubnetSite*> SubnetSiteSet::listNewAndRefinedSubnets()
{
    list<SubnetSite*> newSubnets;
    for(list<SubnetSite*>::iterator it = siteList.begin(); it != siteList.end(); ++it)
    {
        SubnetSite *ss = (*it);
        if(ss->isANewSubnet())
        {
            newSubnets.push_back(ss);
            ss->markAsKnownSubnet();
        }
    }
    return newSubnets;
}
